...
/*   By: zhu <marvin@42.fr>                         +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2016/08/28 23:10:10 by zhu               #+#    #+#             */
/*   Updated: 2016/08/28 23:10:33 by zhu              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "header.h"

char	ft_rush_condition0(int x, int y, int x_ind, int y_ind)
{
	if (x_ind == 0 && y_ind == 0)
		return ('o');
...
char	*char_rush00(int x, int y, int x_i, int y_i)
{
	...
	if (x > 0 && y > 0)
	{
		...
		while (y_i < y)
		{
			while (x_i < x)
			{
				output[j] = ft_rush_condition0(x, y, x_i, y_i);
				x_i++;
				j++;
			}
			if (!(x <= 0 || y <= 0))
